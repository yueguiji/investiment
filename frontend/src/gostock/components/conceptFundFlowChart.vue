...
async function loadAllData() {
  ...
  try {
    ...
    if (isToday.value) {
      await fetchLatestSnapshotIfNeeded()
    }
    // 按日期获取概念排名
    let res = await GetConceptFundFlowTopListByDate(date, 500)
    if (!res || !Array.isArray(res) || res.length === 0) {
      await fetchLatestSnapshotIfNeeded()
      res = await GetConceptFundFlowTopListByDate(date, 500)
    }
    if (!res || !Array.isArray(res) || res.length === 0) {
      topList.value = []
      stopPlay()
    ...
    const actualDate = getDateFromSnapTime(res[0]?.snapTime)
    if (actualDate && actualDate !== selectedDate.value) {
...
async function fetchLatestSnapshotIfNeeded() {
  const now = Date.now()
  if (now - lastFetchAttemptAt < 30000) return
  lastFetchAttemptAt = now
  try {
    await FetchConceptFundFlowNow()
  } catch (e) {
    console.error('FetchConceptFundFlowNow error:', e)
  }
}

function getDateFromSnapTime(snapTime: string): string {
  if (!snapTime || typeof snapTime !== 'string' || snapTime.length < 10) return ''
  return snapTime.substring(0, 10)
}
