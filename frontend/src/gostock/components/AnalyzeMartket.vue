...
const gaugeInstanceRef = ref(null)
...
onUnmounted(()=>{
  ...
  if (treemapChartRef.value) {
    treemapChartRef.value.dispose()
    treemapChartRef.value = null
  }
  if (gaugeInstanceRef.value) {
    gaugeInstanceRef.value.dispose()
    gaugeInstanceRef.value = null
  }
})
...
function  handleChart(){
  ...
  AnalyzeSentimentWithFreqWeight("").then((res) => {
    ...
    const treemapchart = treemapChartRef.value ?? echarts.init(chartRef.value);
    const gaugeChart = gaugeInstanceRef.value ?? echarts.init(gaugeChartRef.value);
    treemapChartRef.value = treemapchart
    gaugeInstanceRef.value = gaugeChart
    if (!res?.frequencies || !res?.result) {
      return
    }
