...
async function loadAllData() {
  ...
  try {
    ...
    // 按日期获取板块排名
    let res = await GetBKFundFlowTopListByDate(date, 500)
    if (!res || !Array.isArray(res) || res.length === 0) {
      await fetchLatestSnapshotIfNeeded()
      res = await GetBKFundFlowTopListByDate(date, 500)
    }
    if (!res || !Array.isArray(res) || res.length === 0) {
      topList.value = []
      stopPlay()
...
async function fetchLatestSnapshotIfNeeded() {
  ...
  try {
    await FetchBKFundFlowNow()
  } catch (e) {
    console.error('FetchBKFundFlowNow error:', e)
  }
}
